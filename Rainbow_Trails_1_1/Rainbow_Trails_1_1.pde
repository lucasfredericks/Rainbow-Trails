...
void setup() {
  ...
  frameComparison = new FrameComparison(rawVideo);
  frameComparison.compare();
}
...
void draw() {
  ...
  rawVideo.endDraw();
  frameComparison.getFrame(rawVideo); // pass the latest frame to the frameComparison object and run an asynchronous comparison
  frameComparison.compare();
  rainbow.loadPixels();
  colorMode(HSB);
